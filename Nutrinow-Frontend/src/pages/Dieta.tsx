...
export default function Dieta() {
  ...
  const [items, setItems] = useState<DietaItem[]>([]);
  ...
  const loadItems = async () => {
    try {
      const res = await apiRequest<{ success: boolean; items: DietaItem[] }>('/dieta-treino?tipo=dieta');
      setItems(res.items || []);
    } catch { /* ok */ }
    setLoading(false);
  };
      ...
      <div className="container" style={{ padding: '2rem 1.5rem', maxWidth: '66rem' }}>
        ...
        {loading ? (
          ...
          <div className="card" style={{ textAlign: 'center', padding: '2rem' }}>
            <Icon name="leaf" size={48} style={{ color: 'var(--muted-foreground)' }} />
            <h3 style={{ margin: '1rem 0 0.5rem' }}>Nenhum plano alimentar</h3>
            <p className="text-muted">Converse com a NutriAI para criar seu plano alimentar personalizado.</p>

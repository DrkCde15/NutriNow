...
(function () {
  ...
  const state = {
    ...
    professionalNotes: { items: [], loading: false, loaded: false, error: "", search: "" },
  ...
  function clearRemoteCaches() {
    ...
    state.professionalNotes = { items: [], loading: false, loaded: false, error: "", search: "" };
  ...
  function defaultAuthenticatedRoute(user = getUser()) {
    return isProfessionalUser(user) ? "/pacientes" : "/";
  }
    ...
    if (authCode && !state.authExchanging) {
      ...
      apiRequest("/auth/exchange-code", {
        ...
        .then((payload) => {
          ...
          routeTo(defaultAuthenticatedRoute(payload.user), true);
        })
        .catch((error) => {
          state.authExchanging = false;
          cleanQueryParams(["auth_code"]);
  ...
  function scheduleRouteLoads(path) {
    ...
    if (path === "/anotacoes" && !state.professionalNotes.loaded && !state.professionalNotes.loading) {
    ...
    if (login) {
      login.addEventListener("submit", async (event) => {
        ...
        try {
          ...
          saveSessionPayload(payload);
          routeTo(defaultAuthenticatedRoute(payload.user));
        } catch (error) {
  ...
  function professionalNoteCategories(role = getUser()?.role) {
    const shared = ["observações", "evolução", "retorno", "orientações"];
    if (role === "nutritionist") {
      return [...shared, "dieta", "refeições", "macros/calorias"];
    }
    return [...shared, "treino", "séries/repetições", "grupos musculares"];
  }

  function patientModalMarkup() {
    if (!state.patientModalOpen) return "";

    const patient = state.editingPatient || {};
    const isEditing = Boolean(state.editingPatient);
    const title = isEditing ? "Editar paciente" : "Cadastrar paciente";
    const subtitle = isEditing
      ? "Atualize as informações usadas no acompanhamento profissional."
      : "Adicione as informações iniciais do seu paciente.";

    return `
      <div class="modal-backdrop premium-modal-backdrop" role="presentation" data-action="close-patient-modal-backdrop">
        <section class="modal-card premium-modal-card professional-modal-card" role="dialog" aria-modal="true" data-modal-card>
          <div class="modal-head professional-modal-head">
            <div>
              <h2>${title}</h2>
          ...
          <form class="form professional-modal-form" data-form="save-patient">
            ${fieldMarkup("Nome do paciente", "nome", "text", { placeholder: "Nome completo", value: patient.nome || "", required: true })}
            <div class="grid-2">
              ${fieldMarkup("Idade", "idade", "number", { placeholder: "Ex: 28", min: "0", value: patient.idade ?? "" })}
              ${fieldMarkup("Objetivo", "objetivo", "text", { placeholder: "Ex: Hipertrofia", value: patient.objetivo || "", required: false })}
            </div>
            <div class="grid-2">
              ${fieldMarkup("Peso (kg)", "peso", "number", { placeholder: "Ex: 75.5", step: "0.1", min: "0", value: patient.peso ?? "", required: false })}
              ${fieldMarkup("Altura (m)", "altura", "number", { placeholder: "Ex: 1.78", step: "0.01", min: "0", value: patient.altura ?? "", required: false })}
            </div>
            <label class="field">
              <span class="label">Observações gerais / histórico</span>
              <textarea class="textarea" name="observacoes" placeholder="Restrições médicas, intolerâncias, hábitos cotidianos...">${escapeHtml(patient.observacoes || "")}</textarea>
            </label>
            <div data-form-error></div>
            <div class="professional-modal-actions">
              ...
              <button type="submit" class="btn btn-primary">${icon("check")} ${isEditing ? "Salvar alterações" : "Salvar cadastro"}</button>
            </div>
          </form>
        </section>
      </div>
  ...
  function patientSelectMarkup(selectedPatientId = "") {
    const selectedId = String(selectedPatientId || "");
    const options = state.patients.items
      .map((patient) => {
        const value = String(patient.id);
        return `<option value="${value}" ${value === selectedId ? "selected" : ""}>${escapeHtml(patient.nome)}</option>`;
      })
      .join("");

    return `
      <label class="field">
        <span class="label" style="margin-bottom:.4rem;">Paciente</span>
        <select class="select" name="patient_id" required autocomplete="off">
  ...
  function noteModalMarkup({ includePatient = false } = {}) {
    if (!state.noteModalOpen) return "";

    const note = state.editingNote || {};
    const isEditing = Boolean(state.editingNote);
    const selectedPatientId = note.patient_id || state.selectedPatient?.id || "";
    const patientField = includePatient && !isEditing ? patientSelectMarkup(selectedPatientId) : "";

    return `
      <div class="modal-backdrop premium-modal-backdrop" role="presentation" data-action="close-note-modal-backdrop">
        <section class="modal-card premium-modal-card professional-modal-card compact" role="dialog" aria-modal="true" data-modal-card>
          <div class="modal-head professional-modal-head">
            <div>
              <h2>${isEditing ? "Editar anotação" : "Nova anotação"}</h2>
              <p class="text-muted">${isEditing ? "Ajuste o registro profissional." : "Registre uma evolução, prescrição ou observação."}</p>
            </div>
            <button class="icon-btn" data-action="close-note-modal" aria-label="Fechar modal">${icon("x")}</button>
          </div>
          <form class="form professional-modal-form" data-form="save-note">
            ${patientField}
            ${selectFieldMarkup(
              "Categoria",
              "note_categoria",
              professionalNoteCategories(),
              note.categoria || "observações",
            )}
            <label class="field">
              <span class="label">Conteúdo da anotação</span>
              <textarea class="textarea" name="note_content" required placeholder="Escreva aqui os detalhes da evolução, prescrição ou orientação...">${isEditing ? escapeHtml(note.content) : ""}</textarea>
            </label>
            <div data-form-error></div>
            <div class="professional-modal-actions">
              <button type="button" class="btn btn-secondary" data-action="close-note-modal">Cancelar</button>
  ...
  function noteCardMarkup(note, { showPatient = false } = {}) {
    const patientName = note.patient_name || state.patients.items.find((patient) => patient.id === note.patient_id)?.nome || "";
    return `
      <article class="note-card professional-note-card">
        <div class="professional-note-head">
          <span class="badge">${escapeHtml(note.categoria || "observações")}</span>
          <small>${formatShortDate(note.criado_em)}</small>
        </div>
        ${showPatient && patientName ? `<strong class="professional-note-patient">${escapeHtml(patientName)}</strong>` : ""}
        <p>${escapeHtml(note.content)}</p>
        <div class="professional-note-actions">
          ${
            showPatient
              ? `<button class="btn btn-secondary btn-sm" data-action="view-note-patient" data-id="${note.patient_id}">${icon("eye")} Prontuário</button>`
              : ""
          }
          <button class="btn btn-secondary btn-sm" data-action="edit-note" data-id="${note.id}">${icon("pencil")} Editar</button>
          <button class="btn btn-secondary btn-sm text-danger" data-action="delete-note" data-id="${note.id}">${icon("trash")} Excluir</button>
        </div>
      </article>
    `;
  }

  function filteredProfessionalNotes() {
    const query = state.professionalNotes.search.trim().toLowerCase();
    if (!query) return state.professionalNotes.items;

    return state.professionalNotes.items.filter((note) => {
      const patientName = note.patient_name || "";
      return [patientName, note.categoria, note.content]
        .join(" ")
        .toLowerCase()
        .includes(query);
    });
  ...
  function anotacoesPage() {
    ...
    const notes = filteredProfessionalNotes();
    const emptyMessage = state.professionalNotes.search
      ? "Nenhuma anotação encontrada para essa busca."
      : "Nenhuma anotação profissional registrada ainda.";
    const notesHtml = state.professionalNotes.loading
      ? `
        <div class="loading-state professional-empty-state">
          <div class="spinner"></div>
          <p class="text-muted">Carregando histórico de anotações...</p>
        </div>
      `
      : notes.length
        ? `<div class="professional-notes-grid">${notes.map((note) => noteCardMarkup(note, { showPatient: true })).join("")}</div>`
        : `
          <div class="empty-state professional-empty-state">
            <span class="feature-icon">${icon("message")}</span>
            <h3>${emptyMessage}</h3>
            <p class="text-muted">Associe anotações a pacientes para manter dieta, treino, evolução e observações em um único histórico.</p>
            <button class="btn btn-primary" data-action="open-note-modal">${icon("plus")} Nova anotação</button>
          </div>
        `;
      ...
      <main class="page-main">
        <div class="container-wide">
          <section class="professional-hero">
            <div>
              <span class="badge">${icon("sparkles")} ${escapeHtml(roleLabel(user.role))}</span>
              <h1>Anotações profissionais</h1>
              <p class="text-muted">Histórico centralizado de orientações, evolução e prescrições associadas aos seus pacientes.</p>
            </div>
            <button class="btn btn-primary" data-action="open-note-modal">${icon("plus")} Nova anotação</button>
          </section>
          ...
          <section class="professional-toolbar">
            <div>
              <span class="text-muted">Total no histórico</span>
              <strong>${state.professionalNotes.items.length}</strong>
            </div>
            <div class="professional-search">
              <span>${icon("search")}</span>
              <input type="text" class="input" placeholder="Buscar por paciente, categoria ou texto..." data-note-search value="${escapeHtml(state.professionalNotes.search)}">
            </div>
          </section>

          ${state.professionalNotes.error ? `<div class="alert alert-error">${icon("alert")} ${escapeHtml(state.professionalNotes.error)}</div>` : ""}
          ${notesHtml}
        </div>
      </main>
      ${noteModalMarkup({ includePatient: true })}
      ${footerMarkup()}
      `,
      "/anotacoes",
    );
  ...
  function pacientesPage() {
          ...
          ${patientList.map((p) => `
            <div class="patient-card card" style="display:flex;flex-direction:column;justify-content:space-between;padding:1.5rem;background:var(--card-bg);border:1px solid var(--border-color);border-radius:1rem;transition:all 0.2s ease;">
              ...
              <div class="patient-card-actions" style="display:flex;gap:0.5rem;margin-top:1rem;border-top:1px solid var(--border-color);padding-top:1rem;">
                ...
                <button class="btn btn-secondary btn-sm icon-btn" data-action="edit-patient" data-id="${p.id}" title="Editar paciente">${icon("pencil")}</button>
    ...
    const modalHtml = patientModalMarkup();
  ...
  function pacienteDetalhePage() {
    ...
    if (state.patientNotes.loading) {
      ...
      notesHtml = `
        <div class="notes-list" style="display:flex;flex-direction:column;gap:1rem;margin-top:1rem;max-height:500px;overflow-y:auto;padding-right:0.5rem;">
          ${state.patientNotes.items.map((note) => noteCardMarkup(note)).join("")}
        </div>
      `;
    }
    ...
    const noteModalHtml = noteModalMarkup();
    const patientModalHtml = patientModalMarkup();
  ...
  async function loadProfessionalNotes(force = false) {
    if (state.professionalNotes.loading || (state.professionalNotes.loaded && !force)) return;
    state.professionalNotes.loading = true;
    state.professionalNotes.error = "";
    render();
    try {
      const result = await apiRequest("/notes");
      state.professionalNotes.items = result.notes || [];
      state.professionalNotes.loaded = true;
    } catch (error) {
      state.professionalNotes.error = getErrorMessage(error, "Erro ao carregar anotações");
    } finally {
      state.professionalNotes.loading = false;
      render();
    }
  }
  ...
  function bindProfessional() {
    ...
    const noteSearch = app.querySelector("[data-note-search]");
    if (noteSearch) {
      noteSearch.addEventListener("input", (e) => {
        const cursorPosition = e.target.selectionStart;
        state.professionalNotes.search = e.target.value;
        render();
        requestAnimationFrame(() => {
          const refreshedInput = app.querySelector("[data-note-search]");
    ...
    app.querySelectorAll('[data-action="edit-patient"]').forEach((btn) => {
      btn.addEventListener("click", () => {
        const id = Number(btn.dataset.id);
        const patient = state.patients.items.find((item) => item.id === id) || (
          state.selectedPatient?.id === id ? state.selectedPatient : null
        );
        if (!patient) return;
    ...
    const savePatientForm = app.querySelector('[data-form="save-patient"]');
    if (savePatientForm) {
      savePatientForm.addEventListener("submit", async (e) => {
        e.preventDefault();
        const data = new FormData(savePatientForm);
        ...
        const editingPatientId = state.editingPatient?.id || null;

        const submitBtn = savePatientForm.querySelector('button[type="submit"]');
        submitBtn.disabled = true;

        try {
          const result = await apiRequest(editingPatientId ? `/patients/${editingPatientId}` : "/patients", {
          ...
          if (editingPatientId && state.selectedPatient?.id === editingPatientId) {
            state.selectedPatient = { ...state.selectedPatient, ...(result.patient || {}) };
          }
          state.patientModalOpen = false;
          state.editingPatient = null;
          await loadPatients(true);
        } catch (err) {
          setFormError(savePatientForm, getErrorMessage(err, "Falha ao salvar paciente"));
        } finally {
    ...
    app.querySelectorAll('[data-action="view-note-patient"]').forEach((btn) => {
      btn.addEventListener("click", () => {
        const id = Number(btn.dataset.id);
        const patient = state.patients.items.find((item) => item.id === id);
        if (!patient) return;
    ...
    app.querySelectorAll('[data-action="edit-note"]').forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const note = [...state.patientNotes.items, ...state.professionalNotes.items].find(item => item.id === noteId);
        if (note) {
